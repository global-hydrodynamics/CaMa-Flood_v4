...
!
      ...
      real,allocatable         ::  slope(:,:)                   !! flood depth [m] (coarse resolution)
!
...
! open hires files
 ...
 1090   continue
      ...
      do iy=1, my
        do ix=1, mx
          if( slope(ix,iy)>=0 )then
            slope(ix,iy)=log10(slope(ix,iy)+0.001)
            slope(ix,iy)=max(minthrs,min(maxthrs,slope(ix,iy)))
            slope(ix,iy)=(slope(ix,iy)-minthrs)/(maxthrs-minthrs)
            if( csize>=1./1000. ) slope(ix,iy)=0.0
          else
            slope(ix,iy)=0.2
          endif
        end do
        ...
        do ix=1, mx2
          ...
          do dx=1, ngrid
            do dy=1, ngrid
              jx=dx+ngrid*(ix-1)
              jy=dy+ngrid*(iy-1)
              out(ix,iy)=out(ix,iy)+slope(jx,jy)
              num=num+1
            end do
          end do
