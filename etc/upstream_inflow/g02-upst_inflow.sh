...
EXP="out_upsinf"                       # experiment name (output directory name)
RDIR=${PWDD}/${EXP}                     # directory to run CaMa-Flood
...
DT=1200                                     # base DT (modified in physics loop by LADPSTP)
...
CNEXTXY="${FMAP}/nextxy_inflow.bin"                # downstream xy (river network map)
...
#============================
#*** 1f. upstream inflow setting
LUPSINF=".true."                            # true for upstream inflow scheme
CINFCSV="${PWDD}/data/mekong_inflow.csv"    # upstream inflow CSV file
IFEQ_INF=1                                  # inflow data frequency (hour)
...
CVARSOUT="outflw,storge,fldfrc,flddph" # list output variable (comma separated)
...
LUPSINF  = ${LUPSINF}                  ! true: upstream inflow scheme
/
&NDIMTIME
...
cat >> ${NMLIST} << EOF
&NUPSINF
CINFCSV  = "${CINFCSV}"                ! inflow CSV file
IFRQ_INF = ${IFRQ_INF}                 ! inflow frequency (hour)
/
EOF
