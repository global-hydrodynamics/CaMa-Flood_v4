#!/bin/sh
#==========================================================
# CaMa-Flood sample go script: levee scheme test
...
EXP="levee_test"                       # experiment name (output directory name)
...
#*** opt. Levee Scheme
LLEVEE=".TRUE."                             # .TRUE. to activate levee scheme 
...
CLEVFRC="${FMAP}/levfrc.bin"                # levee unprotected fraction [0-1]
CLEVHGT="${FMAP}/levhgt.bin"                # levee height [m]
...
CVARSOUT="rivout,rivsto,rivdph,rivvel,fldout,fldsto,flddph,fldfrc,fldare,sfcelv,outflw,storge,pthflw,pthout,maxsto,maxflw,maxdph,levsto,levdph" # list output variable (comma separated)
...
LLEVEE   = ${LLEVEE}                   ! true: activate levee scheme  (under development)
...
cat >> ${NMLIST} << EOF
&NLEVEE
CLEVFRC  = "${CLEVFRC}"                ! Levee unprotectdd fraction
CLEVHGT  = "${CLEVHGT}"                ! Levee Height
/
EOF
