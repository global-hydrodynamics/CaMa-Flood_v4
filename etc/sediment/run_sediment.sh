...
BASE="/Users/yamadai/work/CaMa_sed/cmf_pkg_hatono/"
# BASE="/data26/hatono/CaMa/cama_git/sediment_review/CaMa-Flood_v4"
...
EXP="glb_15min_sediment"                    # experiment name (output directory name)
RDIR=${BASE}/out/${EXP}                     # directory to run CaMa-Flood
EXE="MAIN_cmf"                              # Execute file name
PROG=${BASE}/src/sediment/${EXE}                     # location of Fortran main program
...
LSEDOUT=".TRUE."                            # .TRUE. to activate sediment flow
...
#CROFDIR="${BASE}/inp/"     # runoff directory
...
CVARSOUT="rivout,rivsto"
#CVARSOUT="rivout,outflw,storge,fldfrc" # list output variable (comma separated)
...
##### for sediment (if LSEDOUT=.true.) #########
# Parameters
lambda="0.4d0"                              # bed porosity 
...
#forcing
sedinput_dir="${BASE}/etc/sediment/input"
sedinput_pre="gprct"
sedinput_suf=".one"
dsylunit="1.d-6"                            # sediment yield conversion unit ( m3/km2 -> m3/m2 )
pyld="0.1d0"                                # coefficients for sediment yield estimation based on sunada and hasegawa(1994)
pyldpc="2.d0"                               #    sediment yield[] = pyld * precipitation[mm/hr]**pyldpc * slope[deg]**pyldc
pyldc="1.d0"                                #

# boundary data
crocdph=""               #${fmap}/rocdph.bin"                # bedrock depth
cslope="${BASE}/etc/sediment/map/slope.bin"                  # slope file
sedD="0.001D0,0.000025D0,0.0000001D0"      # sediment diameter for considered grain sizes
#sedD="0.000025D0"
csedfrc="${BASE}/etc/sediment/map/sedfrc.bin"

#output data
#csedsout="sedout,sedcon,sedinp,bedout,netflw,layer,deplyr01,deplyr02"
...
do 
  ...
  if [ ${SPINUP} -eq 0 ];then
    ...
    sedrest_infile="sedrest${CYR}010100.bin"
  fi
...
LSEDOUT  = ${LSEDOUT}                  ! true: activate sediment transport (under development)
...
/
&sediment_input
sedinput_dir = "${sedinput_dir}"
sedinput_pre = "${sedinput_pre}"
sedinput_suf = "${sedinput_suf}"
cslope = "${cslope}"
dsylunit = ${dsylunit}
pyld = ${pyld}
pyldc = ${pyldc}
...
/
&sediment_restart
sedrest_infile = "${sedrest_infile}"
sedrest_outpre = "${sedrest_outpre}"
ifrq_rst_sed = ${ifrq_rst_sed}
/
EOF
...
then
  if [ ${ISP} -le ${NSP} ];
    ...
    cp -f sedrest${CYR1}010100.bin sedrest${CYR}010100.bin-sp${ISP}         2> /dev/null
    mv -f sedrest${CYR1}010100.bin sedrest${CYR}010100.bin                  2> /dev/null

    cp -f sedrest${CYR1}010100.nc sedrest${CYR}010100.nc-sp${ISP}           2> /dev/null
    mv -f sedrest${CYR1}010100.nc sedrest${CYR}010100.nc                    2> /dev/null
    ...
    mv -f ./sedrest${CYR}010100.bin-sp${ISP}      ${CYR}-sp${ISP}  2> /dev/null
    mv -f ./sedrest${CYR}010100.nc-sp${ISP}       ${CYR}-sp${ISP}  2> /dev/null
