...
EXP="test5-moz_06min_sealev"                # experiment name (output directory name)
...
DT=600                                      # base DT (modified in physics loop by LADPSTP)
...
#============================
#*** 1c. simulation time
YSTA=2019                                   # start year ( from YSTA / Jan  1st _ 00:00)
YEND=$YSTA                                   # end   year (until YEND / Dec 31st _ 24:00)
...
#* input restart file
LRESTART=".TRUE."                           # TRUE. to use restart initial condition
CRESTSTO="${INPDIR}/restart2019010100.nc"   # input restart FIle
...
CROFPRE=""                                    # runoff prefix/suffix  
CROFCDF="${INPDIR}/era5_ro_20190101-20190501_daily_moz.nc"  # netCDF runoff file
...
CINPMAT=${FMAP}/inpmat_ERA5_ro_moz.bin     # runoff input matrix for interporlation
...
LSEALEV=".TRUE."                           # .TRUE. to activate dynamic sea level 
LSEALEVCDF=".TRUE."
CSEALEVDIR=""                             # Sea level boundary DIRECTORY
CSEALEVPRE=""                             # Sea level boundary PREFIX
CSEALEVSUF=""                              # Sea level boundary SUFFIX
CSEALEVCDF="${INPDIR}/gtsm_era5_ro_20190101-20190501_moz.nc" # * Sea level netCDF file name
CVNSEALEV="waterlevel"                     # * Sea Level netCDF variable name
SYEARSL=2019                              # see (3) set each year
SMONSL=1                                    # see (3) set each year
SDAYSL=1                                    # see (3) set each year
SHOURSL=0                                   # see (3) set each year
CSLMAP="${FMAP}/GTSM_EVENT.txt"            # sea level sta->XY conversion table
IFRQ_SL=10                                 # sea level boundary update frequency (min)
...
CVARSOUT="outflw,storge,maxdph,maxflw,flddph,fldsto,fldare,pthout" # list output variable (comma separated)
