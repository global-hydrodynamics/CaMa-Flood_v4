...
!
      ...
      real,allocatable         ::  flddph(:,:),levdph(:,:)                   !! flood depth [m] (coarse resolution)
!
      real,allocatable         ::  flood(:,:),protect(:,:)                    !! downscaled flood depth [m]
!
      character*256            ::  mapdir, hires
      parameter                   (mapdir='./map/')              !! map directory (please make a symbolic link)
      character*256            ::  fnextxy, fflood, fprotect, rfile
      character*256            ::  fflddph, flevdph
      integer                  ::  trec
      character*256            ::  buf
      integer                  ::  ios, ioslev

      character(len=128)       :: fctl, fbin_base
! ===============================================
! downscale target domain
      ...
      call getarg(7,flevdph)    !! downscale file

      call getarg(8,fflood)     !! output file
      call getarg(9,buf)        !! downscale file record number
      if( buf/='' )then
        read(buf,*) trec
      else
        trec=1
...
! ==========
      ...
      allocate(nextXX(nXX,nYY),flddph(nXX,nYY),levdph(nXX,nYY))
      allocate(flood(mx,my),protect(mx,my))
      flood(:,:)=-9999

      fprotect='./downscale/protect_pix.bin'
      open(11, file=fprotect, form='unformatted', access='direct', recl=4*mx*my)
      read(11,rec=1) protect
      close(11)
...
! ===============================================
      ...
      open(12, file=fflddph, form='unformatted', access='direct', recl=4*nXX*nYY,status='old',iostat=ios)
      read(12,rec=trec) flddph
      close(12)
      do iYY=1, nYY
        do iXX=1, nXX
      ...
      open(12, file=flevdph, form='unformatted', access='direct', recl=4*nXX*nYY,status='old',iostat=ioslev)
      if( ioslev==0 )then
        read(12,rec=trec) levdph
        close(12)
        do iYY=1, nYY
          do iXX=1, nXX
            if( levdph(iXX,iYY)==1.e20 )then
              levdph(iXX,iYY)=-9999
            endif
          end do
        end do
...
! open hires files
          ...
          do ix=1, nx
            if( lon(ix)>west2 .and. lon(ix)<east2 .and. lat(iy)>south2 .and. lat(iy)<north2 )then
              ...
              if( catmXX(ix,iy)>0 )then
                ...
                iXX=catmXX(ix,iy)
                iYY=catmYY(ix,iy)
                if( ioslev==0 .and. protect(jx,jy)>=5 )then
                  if( levdph(iXX,iYY)>flddif(ix,iy) )then
                    flood(jx,jy)=levdph(iXX,iYY)-flddif(ix,iy)+100
                  endif
                else
 ...
 1090   continue
      ...
      !=========================================================
      ! Write GrADS CTL for downscaled file (hand.bin)
      !=========================================================
      fctl = './downscale/flood_lev.ctl'
      fbin_base = 'flood_lev.bin'   ! ctl 内で参照する bin 名（必要なら変更）
    
      open(31, file=fctl, status='replace', form='formatted')
    
      write(31,'(a)') 'dset ^'//trim(fbin_base)
      write(31,'(a)') 'undef -9999'
      write(31,'(a)') 'title Dummy'
      write(31,'(a)') 'options yrev little_endian'
    
      ! XDEF/YDEF：開始点は格子中心にするのが一般的
      ! ここでは west2+0.5*csize, south2+0.5*csize を採用
      write(31,'(a,i0,a,f15.8,1x,f15.8)') 'xdef ', mx, ' linear ', west2 + 0.5d0*csize, csize
      write(31,'(a,i0,a,f15.8,1x,f15.8)') 'ydef ', my, ' linear ', south2 + 0.5d0*csize, csize
    
      write(31,'(a)') 'tdef 1 linear 00Z01jan2000 1yr'
      write(31,'(a)') 'zdef 1 linear 1 1'
      write(31,'(a)') 'vars 1'
      write(31,'(a)') 'var  1 99 **'
      write(31,'(a)') 'ENDVARS'
