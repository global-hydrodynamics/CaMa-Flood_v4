...
! open hires files
          ...
          do ix=1, nx
            if( lon(ix)>west2 .and. lon(ix)<east2 .and. lat(iy)>south2 .and. lat(iy)<north2 )then
              ...
              if( catmXX(ix,iy)>0 )then
                ...
                iXX=catmXX(ix,iy)
                iYY=catmYY(ix,iy)
                if( levfrc(iXX,iYY)<0 )then
                  protect(jx,jy)=0           !! no protection
                elseif( catmZZ(ix,iy)>levfrc(iXX,iYY)*100 )then  !! protected pixel
                  protect(jx,jy)=5
                endif
 ...
 1090   continue
      ...
      !=========================================================
      fctl = './downscale/hand.ctl'
      fbin_base = 'hand.bin'   ! ctl 内で参照する bin 名（必要なら変更）
      ...
      !=========================================================
      ! Write GrADS CTL for downscaled file
      !=========================================================
      fctl = './downscale/protect_pix.ctl'
      fbin_base = 'protect_pix.bin'   ! ctl 内で参照する bin 名（必要なら変更）
