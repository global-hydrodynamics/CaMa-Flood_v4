...
! river network map
      ...
      integer,allocatable ::  mask(:,:)               !! downstream xy
...
! read river network and input file dimentions
      ...
      allocate(nextx(nx,ny),nexty(nx,ny),rivseq(nx,ny),ctmare(nx,ny),mask(nx,ny))
...
! ============================================================
      ...
      cnextxy='./nextxy_tonle.bin'
      open(11,file=cnextxy,form='unformatted',access='direct',recl=4*nx*ny,iostat=ios)
      write(11,rec=1) nextx
      write(11,rec=2) nexty
      close(11)
