...
subroutine LayerUpdater_reset_update( &
    &   self)
        class(LayerUpdater), intent(inout) :: &
        &   self
    self%min_oldlyr(:) = 1.e20_JPRB
    self%max_oldlyr(:) = -1.e20_JPRB
...
&   oldsto, newsto)
    ...
    subroutine validate_updater
        ...
        real(kind=JPRB) :: &
        &   vol
        allocate(oldsto_tmp(size(oldsto)), source=0.0_JPRB)
        allocate(newsto_tmp(size(newsto)), source=0.0_JPRB)
        do i = 1, self%size
...
subroutine divlyr(lyr, ttllyr, maxlyr)
    ...
    real(kind=JPRB), intent(out) :: lyr(:)
    real(kind=JPRB), intent(in)  :: ttllyr, maxlyr(:)
    real(kind=JPRB) :: ttlrst
    integer ilyr, nlyr
    lyr(:) = 0.0_JPRB
    if (ttllyr == 0.0_JPRB) return
    nlyr   = size(maxlyr)
    ttlrst = ttllyr
    do ilyr = 1, nlyr
...
function cntlyr(lyrval) result(nlyr)
    ! count layer number
    integer nlyr
    real(kind=JPRB), intent(in) :: lyrval(:)
    integer ilyr
    if (sum(lyrval) == 0.0_JPRB) then
        nlyr = 0
        return
    endif
    nlyr = size(lyrval)
    do ilyr = 1, size(lyrval)
        if (lyrval(ilyr) == 0.0_JPRB) then
            nlyr = ilyr - 1
            exit
        endif
    enddo
...
subroutine sftlyr(lyrval)
    ...
    real(kind=JPRB), intent(inout) :: lyrval(:)
    real(kind=JPRB), allocatable   :: lyrtmp(:)
    integer ilyr, nlyr, tmp
    nlyr = size(lyrval)
    allocate(lyrtmp(nlyr)); lyrtmp(:) = 0.0_JPRB
    tmp = 0
    do ilyr = 1, nlyr
        if (lyrval(ilyr) > 0.0_JPRB) then
            tmp = tmp + 1
...
subroutine rdclyr_upper(lyrval, rdclyr, rdcval)
    ...
    real(kind=JPRB), intent(inout) :: lyrval(:) ! [X e.g. m3] layer
    real(kind=JPRB), intent(out)   :: rdclyr(:) ! [X e.g. m3] reduction of each layer
    real(kind=JPRB), intent(in)    :: rdcval    ! [X e.g. m3] total reduction
    real(kind=JPRB) :: rstrdc
    integer ilyr, nlyr
    nlyr   = size(lyrval)
    rdclyr(:) = 0.0_JPRB
    rstrdc = rdcval
...
function ttlout(lyrmat, lyrout) result(matout)
    real(kind=JPRB) :: matout ! [X*m3/s or X*m3] outflow from all layers
    real(kind=JPRB), intent(in)  :: &
...
function get_higher_layer(lyrelv, trgelv) result(uprlyr)
    ...
    integer             :: uprlyr
    real(kind=JPRB), intent(in) :: lyrelv(:), trgelv
    integer ilyr, nlyr
...
subroutine eddmix( lyrx, lyrthk, veredd, setvel, dt )
    real(kind=JPRB), intent(inout) :: lyrx(:) ! [X] material concentration
    real(kind=JPRB), intent(in)    :: &
...
subroutine eddmix_nosettle__(lyrx, lyrthk, veredd, dt)
    ...
    &   veredd(:), & ! [?] vertical eddy coefficient, flux = veredd * dMat
    &   dt           ! [s]   time step width
    real(kind=JPRB), allocatable :: A(:,:), b(:), lyrdx(:) ! [A][lyrdx] = [b]
    integer :: ilyr, nlyr

    nlyr = size(lyrx)
    if ( nlyr == 1 ) return

    allocate( A(nlyr,nlyr) ); A(:,:) = 0.0_JPRB
    allocate( b(nlyr)      ); b(:)   = 0.0_JPRB
    do ilyr = 1, nlyr
        A(ilyr,ilyr) = 1.0_JPRB
    enddo
    call add_eddmix_nosettle
    allocate( lyrdx(nlyr) ); lyrdx(:) = 0.0_JPRB
...
subroutine eddmix_settle__(lyrx, lyrthk, veredd, setvel, dt)
    ...
    &   setvel,    & ! [m/s] settling velocity
    &   dt           ! [s]   time step width
    real(kind=JPRB), allocatable :: A(:,:), b(:)
    integer :: ilyr, nlyr
...
!    deallocate(lyrdx)
    ...
    subroutine add_eddmix_settle
        real(kind=JPRB) :: pe, fpe, dz, k, c, edd, set ! fpe = 1/exp(pe)-1
        real(kind=JPRB) :: pe_max = 46.0_JPRB ! exp(pe) = 10**20
        integer :: ilyr
        do ilyr = 1, nlyr-1 ! b/w ilyr and ilyr+1
            if ( abs(veredd(ilyr)) <= 1.e-20_JPRB ) then
                fpe = 0.0_JPRB
            else
                pe  = max( -10.0_JPRB, min( setvel * lyrthk(ilyr) / veredd(ilyr), pe_max ) )
    !            fpe = max( 0.d0, (1.d0 - 0.1d0 * abs(pe) ) ** 5 / pe )
                fpe = exp(pe) - 1.0_JPRB
                if ( fpe /= 0.0_JPRB ) then
                    fpe = 1.0_JPRB / fpe
                else
                    fpe = 0.0_JPRB
                endif
            endif
            dz  = ( lyrthk(ilyr) + lyrthk(ilyr+1) ) * 0.5_JPRB
            k   = setvel * dt * ( 1.0_JPRB + fpe ) / dz
!            k   = setvel * dt * fpe / dz
...
!            b(ilyr) = b(ilyr) + edd + set
        enddo
        do ilyr = 2, nlyr ! b/w ilyr and ilyr-1
            if ( abs(veredd(ilyr-1)) <= 1.e-20_JPRB ) then
                fpe = 0.0_JPRB
            else
                pe  = max( -10.0_JPRB, min( setvel * lyrthk(ilyr) / veredd(ilyr-1), pe_max ) )
    !            fpe = max( 0.d0, (1.d0 - 0.1d0 * abs(pe) ) ** 5 / pe )
                fpe = exp(pe) - 1.0_JPRB
                if ( fpe /= 0.0_JPRB ) then
                    fpe = 1.0_JPRB / fpe
            ...
            dz  = ( lyrthk(ilyr-1) + lyrthk(ilyr) ) * 0.5_JPRB
            k   = setvel * dt * fpe / dz
