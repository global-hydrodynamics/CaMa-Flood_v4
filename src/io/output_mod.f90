...
! ==============================================================================================
subroutine write_output(now_t)
    integer(kind=JPIM), intent(in) :: now_t
    integer :: i

    write(LOGNAM, '(a,I0)') '[output_mod/write_output] now_t = ', now_t
    if (.not. allocated(confs)) return

    do i = 1, size(confs)
        if (.not. confs(i)%write_needed(now_t)) cycle
        call write_one(i)
        call confs(i)%set_next()
    enddo
...
subroutine write_one(idx)
    ...
    write(LOGNAM, '(a,1x,a)') '  [output_mod/write_one] ', trim(confs(idx)%get_item())
...
integer function ensure_registered_1d(item, n1) result(idx)
    ...
    call init_OutputConf(item, is_found, conf, 0) ! t=0 for initialization
