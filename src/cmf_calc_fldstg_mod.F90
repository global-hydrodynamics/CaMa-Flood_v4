...
REAL(KIND=JPRD),SAVE       :: PSTO_all
REAL(KIND=JPRB),SAVE       :: DSTO_all, DSTO_add, DSTO_fil, DWTH_add, DWTH_fil, DDPH_fil, DWTH_inc
!$OMP THREADPRIVATE        (I,DSTO_all, DSTO_add, DSTO_fil, DWTH_add, DWTH_fil, DDPH_fil, DWTH_inc,PSTO_all)
!================================================
...
DO ISEQ=1, NSEQALL
!
  PSTO_all = P2RIVSTO(ISEQ,1) + P2FLDSTO(ISEQ,1)
  DSTO_all = REAL( PSTO_all, KIND=JPRB)
  P0GLBSTOPRE2     = P0GLBSTOPRE2 + PSTO_all

  IF( PSTO_all > D2RIVSTOMAX(ISEQ,1) )THEN
    I=1
    DSTO_fil = D2RIVSTOMAX(ISEQ,1)
    DWTH_fil = D2RIVWTH(ISEQ,1)
    DDPH_fil = 0._JPRB
    DWTH_inc = D2GRAREA(ISEQ,1) / D2RIVLEN(ISEQ,1) * DFRCINC
    DO WHILE( PSTO_all > D2FLDSTOMAX(ISEQ,1,I) .AND. I<=NLFP)
...
&      ( DWTH_fil**2._JPRB + 2._JPRB * DSTO_add / D2RIVLEN(ISEQ,1) / D2FLDGRD(ISEQ,1,I) )**0.5_JPRB
    ...
    P2RIVSTO(ISEQ,1) = MIN(P2RIVSTO(ISEQ,1),PSTO_all)

    D2RIVDPH(ISEQ,1) = REAL(P2RIVSTO(ISEQ,1),JPRB) / D2RIVLEN(ISEQ,1) / D2RIVWTH(ISEQ,1)
!
    P2FLDSTO(ISEQ,1) = PSTO_all - P2RIVSTO(ISEQ,1)
  ...
  ELSE
    P2RIVSTO(ISEQ,1) = PSTO_all
    D2RIVDPH(ISEQ,1) = DSTO_all / D2RIVLEN(ISEQ,1) / D2RIVWTH(ISEQ,1)
...
LOGICAL,SAVE               :: Mask
REAL(KIND=JPRD),SAVE       :: PSTO_all
REAL(KIND=JPRB),SAVE       :: DSTO_all, DWTH_fil, DSTO_add, DWTH_add, Dmod, Dtmp
!================================================
P0GLBSTOPRE2=0._JPRD
P0GLBSTONEW2=0._JPRD
P0GLBRIVSTO =0._JPRD
...
!$OMP PARALLEL DO PRIVATE(I,ILFP,PSTO_all) REDUCTION(+:P0GLBSTOPRE2)
DO ISEQ=1, NSEQALL
  PSTO_all = P2RIVSTO(ISEQ,1) + P2FLDSTO(ISEQ,1)
  P0GLBSTOPRE2 = P0GLBSTOPRE2 + PSTO_all
  ...
  IF (PSTO_all > D2RIVSTOMAX(ISEQ,1)) THEN
    ...
    DDPH_fil(ISEQ)=0._JPRB
    DO ILFP=1, NLFP
      IF (PSTO_all <= D2FLDSTOMAX(ISEQ,1,ILFP)) EXIT
...
!$OMP PARALLEL DO SIMD PRIVATE(PSTO_all,DSTO_all,DWTH_fil,DSTO_add,DWTH_add,Dtmp,Dmod,Mask)
DO ISEQ=1, NSEQALL
  PSTO_all = P2RIVSTO(ISEQ,1) + P2FLDSTO(ISEQ,1)
  DSTO_all = REAL( PSTO_all, KIND=JPRB)
  ...
  DSTO_add =  DSTO_all - DSTO_fil(ISEQ)
...
INTEGER(KIND=JPIM),SAVE    :: ISEQ, I
REAL(KIND=JPRD),SAVE       :: PSTO_all
REAL(KIND=JPRB),SAVE       :: DSTO_all, DSTO_add, DWTH_add
!================================================
P0GLBRIVSTO=0._JPRD
P0GLBFLDSTO=0._JPRD
P0GLBFLDARE=0._JPRD
...
!$OMP PARALLEL DO SIMD PRIVATE(DSTO_all,PSTO_all) REDUCTION(+:P0GLBSTOPRE2)
DO ISEQ=1, NSEQALL
  PSTO_all = P2RIVSTO(ISEQ,1) + P2FLDSTO(ISEQ,1)
  P0GLBSTOPRE2     = P0GLBSTOPRE2 + PSTO_all
...
DO I=1, NLFP
!$OMP PARALLEL DO SIMD PRIVATE(DSTO_all,DSTO_add,DWTH_add,PSTO_all) 
  DO ISEQ=1, NSEQALL
    PSTO_all = P2RIVSTO(ISEQ,1) + P2FLDSTO(ISEQ,1)
    DSTO_all = REAL( PSTO_all, KIND=JPRB )

    DSTO_add = DSTO_all - D2STO_fil(ISEQ,1)
...
!$OMP PARALLEL DO SIMD PRIVATE(DSTO_all,DSTO_add,PSTO_all) 
DO ISEQ=1, NSEQALL
  PSTO_all = P2RIVSTO(ISEQ,1) + P2FLDSTO(ISEQ,1)
  DSTO_all = REAL( PSTO_all, KIND=JPRB)
  DSTO_add = DSTO_all - D2STO_fil(ISEQ,1)
...
!$OMP PARALLEL DO SIMD PRIVATE(PSTO_all) 
DO ISEQ=1, NSEQALL
  PSTO_all = P2RIVSTO(ISEQ,1) + P2FLDSTO(ISEQ,1)
