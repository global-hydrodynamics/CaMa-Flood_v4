...
USE YOS_CMF_MAP,        ONLY: I2NEXTX,I2NEXTY, I2REGION, REGIONALL,REGIONTHIS, &
                            & I1SEQX, I1SEQY,  I1NEXT,  I2VECTOR, B1LON,    B1LAT,      &
...
IF( WEST>=-180._JPRB .and. EAST<=360._JPRB .and. SOUTH>=-180._JPRB .and. NORTH<=180._JPRB )THEN  !! bugfix_v396a
  !$OMP PARALLEL DO
  DO IX=1,NX
    B1LON(IX)=WEST +(DBLE(IX)-0.5D0)*(EAST-WEST)  /DBLE(NX)
  ENDDO
  !$OMP END PARALLEL DO
  !$OMP PARALLEL DO
  DO IY=1,NY
    B1LAT(IY)=NORTH-(DBLE(IY)-0.5D0)*(NORTH-SOUTH)/DBLE(NY)
  ENDDO
  !$OMP END PARALLEL DO
ENDIF
...
CALL NCERROR ( NF90_GET_VAR(NCID,VARID,B1LAT),'reading data' )

CALL NCERROR ( NF90_INQ_VARID(NCID,'lon',VARID),'getting id' )
CALL NCERROR ( NF90_GET_VAR(NCID,VARID,B1LON),'reading data' )
...
USE YOS_CMF_MAP,    ONLY: B2NXTDST, B2GRAREA, B2ELEVTN, B2RIVLEN, &
                        & B2RIVWTH, B2RIVHGT, B2FLDHGT, B2RIVELV, &
                        & B2FLDGRD, B2RIVMAN, B2RIVSTOMAX, B2FLDSTOMAX,  &
                        & BFRCINC,  NSEQALL,  NSEQMAX, B2MEANSL, B2DWNELV, &
                        & B2GDWDLY, I2MASK
IMPLICIT NONE
!================================================
WRITE(LOGNAM,*) ""
...
! *** 1. ALLOCATE ARRAYS
ALLOCATE( B2GRAREA(NSEQMAX,1) )
ALLOCATE( B2ELEVTN(NSEQMAX,1) )
ALLOCATE( B2NXTDST(NSEQMAX,1) )
ALLOCATE( B2RIVLEN(NSEQMAX,1) )
ALLOCATE( B2RIVWTH(NSEQMAX,1) )
ALLOCATE( B2RIVHGT(NSEQMAX,1) )
ALLOCATE( B2FLDHGT(NSEQMAX,1,NLFP) )
ALLOCATE( B2RIVMAN(NSEQMAX,1) )
ALLOCATE( B2MEANSL(NSEQMAX,1) )
ALLOCATE( B2DWNELV(NSEQMAX,1) )
ALLOCATE( B2GDWDLY(NSEQMAX,1) )
...
ALLOCATE(B2RIVSTOMAX(NSEQMAX,1))
ALLOCATE(B2RIVELV(NSEQMAX,1))

IF ( LFPLAIN ) THEN
  B2RIVSTOMAX(:,:) = B2RIVLEN(:,:) * B2RIVWTH(:,:) * B2RIVHGT(:,:)
ELSE
  WRITE(LOGNAM,*) 'TOPO_INIT: no floodplain (rivstomax=1.D18)'
  B2RIVSTOMAX(:,:) = 1.E18
ENDIF
B2RIVELV(:,:) = B2ELEVTN(:,:) - B2RIVHGT(:,:)

!*** 3b. Calc Channel Parameters
WRITE(LOGNAM,*) 'TOPO_INIT: calc floodplain parameters'

ALLOCATE(B2FLDSTOMAX(NSEQMAX,1,NLFP))
ALLOCATE(B2FLDGRD(NSEQMAX,1,NLFP))
CALL SET_FLDSTG
...
USE CMF_UTILS_MOD,       ONLY: MAPR2VECB, CONV_END,  INQUIRE_FID
IMPLICIT NONE
!* local variables
INTEGER(KIND=JPIM)          :: ILFP
REAL(KIND=JPRM),ALLOCATABLE :: R2TEMP(:,:)
REAL(KIND=JPRB),ALLOCATABLE :: B2TEMP(:,:)
!================================================
ALLOCATE(R2TEMP(NX,NY))
ALLOCATE(B2TEMP(NSEQMAX,1))
...
CALL MAPR2VECB(R2TEMP,B2GRAREA)
CLOSE(TMPNAM)
...
READ(TMPNAM,REC=1) R2TEMP(:,:)
  IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
CALL MAPR2VECB(R2TEMP,B2ELEVTN)
CLOSE(TMPNAM)
...
READ(TMPNAM,REC=1) R2TEMP(:,:)
  IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
CALL MAPR2VECB(R2TEMP,B2NXTDST)
CLOSE(TMPNAM)
...
READ(TMPNAM,REC=1) R2TEMP(:,:)
  IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
CALL MAPR2VECB(R2TEMP,B2RIVLEN)
CLOSE(TMPNAM)
...
DO ILFP=1,NLFP
  ...
  IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
  CALL MAPR2VECB(R2TEMP,B2TEMP)
  B2FLDHGT(:,:,ILFP)= B2TEMP(:,:)
ENDDO
CLOSE(TMPNAM)
...
READ(TMPNAM,REC=1) R2TEMP(:,:)
  IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
CALL MAPR2VECB(R2TEMP,B2RIVHGT)
CLOSE(TMPNAM)
...
READ(TMPNAM,REC=1) R2TEMP(:,:)
  IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
CALL MAPR2VECB(R2TEMP,B2RIVWTH)
CLOSE(TMPNAM)
...
READ(TMPNAM,REC=1) R2TEMP(:,:)
  IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
CALL MAPR2VECB(R2TEMP,B2RIVMAN)
CLOSE(TMPNAM)
...
IF( LGDWDLY )THEN
  ...
  READ(TMPNAM,REC=1) R2TEMP(:,:)
    IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
  CALL MAPR2VECB(R2TEMP,B2GDWDLY)
  CLOSE(TMPNAM)
ENDIF
...
IF( LMEANSL ) THEN
  ...
  READ(TMPNAM, REC=1) R2TEMP(:,:)
    IF( LMAPEND ) CALL CONV_END(R2TEMP,NX,NY)
  CALL MAPR2VECB(R2TEMP, B2MEANSL)
  CLOSE(TMPNAM)
ENDIF
...
#ifdef UseCDF_CMF
USE NETCDF 
USE CMF_UTILS_MOD,            ONLY: NCERROR,MAPR2VECB
USE YOS_CMF_MAP,              ONLY: B2ELEVSLOPE     !! only used in ECMWF
IMPLICIT NONE
!* local variables
...
DO ILEV=1,NLFP
  ...
  CALL MAPR2VECB(R2TEMP,B2TEMP)
  B2FLDHGT(:,:,ILEV)=B2TEMP(:,:)
ENDDO

CALL NCERROR( NF90_CLOSE(NCID))

IF ( LSLOPEMOUTH ) THEN
  ALLOCATE( B2ELEVSLOPE(NSEQMAX,1) )
  ...
  ELSE
    CALL NCERROR ( NF90_GET_VAR(NCID,VARID,R2TEMP),'reading data' ) 
  ENDIF 
  CALL MAPR2VECB(R2TEMP,B2ELEVSLOPE)
ENDIF
...
REAL(KIND=JPRB),SAVE     ::  BSTONOW
REAL(KIND=JPRB),SAVE     ::  BSTOPRE
REAL(KIND=JPRB),SAVE     ::  BHGTPRE
REAL(KIND=JPRB),SAVE     ::  BWTHINC
!$OMP THREADPRIVATE       (I,BSTONOW,BSTOPRE,BHGTPRE,BWTHINC)
!================================================
B2FLDSTOMAX(:,:,:) = 0._JPRB
B2FLDGRD(:,:,:)    = 0._JPRB
BFRCINC=dble(NLFP)**(-1.)
...
!$OMP PARALLEL DO
DO ISEQ=1, NSEQALL
  BSTOPRE = B2RIVSTOMAX(ISEQ,1)
  BHGTPRE = 0._JPRB
  BWTHINC = B2GRAREA(ISEQ,1) * B2RIVLEN(ISEQ,1)**(-1.) * BFRCINC
  DO I=1, NLFP
    BSTONOW = B2RIVLEN(ISEQ,1) * ( B2RIVWTH(ISEQ,1) + BWTHINC*(DBLE(I)-0.5) ) * (B2FLDHGT(ISEQ,1,I)-BHGTPRE)
    B2FLDSTOMAX(ISEQ,1,I) = BSTOPRE + BSTONOW
    B2FLDGRD(ISEQ,1,I) = (B2FLDHGT(ISEQ,1,I)-BHGTPRE) * BWTHINC**(-1.)
    BSTOPRE = B2FLDSTOMAX(ISEQ,1,I)
    BHGTPRE = B2FLDHGT(ISEQ,1,I)
  END DO
END DO
!$OMP END PARALLEL DO
