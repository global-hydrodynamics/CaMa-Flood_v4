...
!! Use MPI: read MPI region map, allocate regions to MPI nodes
  ...
  REGIONALL=1
!$OMP PARALLEL DO SIMD REDUCTION(max:REGIONALL)
  DO IY=1, NY
    DO IX=1, NX
...
DO IREGION=1, REGIONALL
  ...
  NSEQMAX_MPI=MAX(NSEQMAX,REGIONGRID(IREGION))  !! maximum nseqall among all MPI region
END DO
...
USE YOS_CMF_MAP,    ONLY: D2NXTDST, D2GRAREA, D2ELEVTN, D2RIVLEN, &
                        ...
                        & DFRCINC,  NSEQALL,  NSEQALL, D2MEANSL, D2DWNELV, &
                        & D2GDWDLY, I2MASK
IMPLICIT NONE
!================================================
WRITE(LOGNAM,*) ""
...
! *** 1. ALLOCATE ARRAYS
ALLOCATE( D2GRAREA(NSEQALL,1) )
ALLOCATE( D2ELEVTN(NSEQALL,1) )
ALLOCATE( D2NXTDST(NSEQALL,1) )
ALLOCATE( D2RIVLEN(NSEQALL,1) )
ALLOCATE( D2RIVWTH(NSEQALL,1) )
ALLOCATE( D2RIVHGT(NSEQALL,1) )
ALLOCATE( D2FLDHGT(NSEQALL,1,NLFP) )
ALLOCATE( D2RIVMAN(NSEQALL,1) )
ALLOCATE( D2MEANSL(NSEQALL,1) )
ALLOCATE( D2DWNELV(NSEQALL,1) )
ALLOCATE( D2GDWDLY(NSEQALL,1) )
ALLOCATE( I2MASK(NSEQALL,1) )
...
ALLOCATE(D2RIVSTOMAX(NSEQALL,1))
ALLOCATE(D2RIVELV(NSEQALL,1))
...
ALLOCATE(D2FLDSTOMAX(NSEQALL,1,NLFP))
ALLOCATE(D2FLDGRD(NSEQALL,1,NLFP))
CALL SET_FLDSTG
...
IF ( LSLOPEMOUTH ) THEN
  ALLOCATE( D2ELEVSLOPE(NSEQALL,1) )
