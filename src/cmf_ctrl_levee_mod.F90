...
USE YOS_CMF_MAP,        ONLY: NSEQALL, NSEQALL
...
! calculate floodplain parameters (without levee, same as SET_FLDSTG)
  ...
  DO I=1, NLFP
    DSTONOW = D2RIVLEN(ISEQ,1) * ( D2RIVWTH(ISEQ,1) + DWTHINC*(REAL(I,KIND=JPRB)-0.5) ) * (D2FLDHGT(ISEQ,1,I)-DHGTPRE)  !! storage increment
...
!$OMP PARALLEL DO SIMD REDUCTION(+:P0GLBSTOPRE2,P0GLBSTONEW2,P0GLBRIVSTO,P0GLBFLDSTO,P0GLBLEVSTO,P0GLBFLDARE)
DO ISEQ=1, NSEQALL
!
...
USE YOS_CMF_MAP,        ONLY: NSEQALL, NSEQALL, NPTHOUT, NPTHLEV, PTH_UPST, PTH_DOWN, PTH_DST, &
