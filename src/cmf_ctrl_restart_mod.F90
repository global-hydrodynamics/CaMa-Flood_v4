...
USE YOS_CMF_PROG,   ONLY: D2RIVSTO,    D2FLDSTO,    B2RIVOUT,    B2FLBOUT,    B2GDWSTO, &
                        & B2RIVOUT_PRE,B2FLBOUT_PRE,B2RIVDPH_PRE,B2FLDSTO_PRE,&
                        & B1PTHFLW,    B1PTHFLW_PRE, &
...
ENDIF
IF( LDAMOUT ) then
  D2DAMSTO(:,:)=0._JPRD   !!! added LDAMOUT
ENDIF
IF( LLEVEE ) then
  D2LEVSTO(:,:)=0._JPRD   !!! added LLEVEE
ENDIF
IF( LGDWDLY ) then
  B2GDWSTO(:,:)=0._JPRD
ENDIF
...
REAL(KIND=JPRD)                 :: D2VEC(NSEQMAX,1)
REAL(KIND=JPRM)                 :: R1PTH(NPTHOUT,NPTHLEV)
REAL(KIND=JPRD)                 :: D1PTH(NPTHOUT,NPTHLEV)
...
RIREC=0
CALL READ_BIN_MAP(D2VEC,TMPNAM,RIREC)
 D2RIVSTO=D2VEC
CALL READ_BIN_MAP(D2VEC,TMPNAM,RIREC)
 D2FLDSTO=D2VEC
...
IMPLICIT NONE
REAL(KIND=JPRD)            :: D2VAR(NSEQMAX,1)
INTEGER(KIND=JPIM)         :: TNAM, IREC
!* local
REAL(KIND=JPRM)            :: R2TEMP(NX,NY)
REAL(KIND=JPRD)            :: D2TEMP(NX,NY)
!=================
IREC=IREC+1
...
USE CMF_UTILS_MOD,    ONLY: NCERROR, MAPD2VECD, MAPD2VECB
IMPLICIT NONE
! local variables
INTEGER(KIND=JPIM)    ::  NCID,VARID
CHARACTER(LEN=256)    ::  CFILE
REAL(KIND=JPRD)       ::  D2TEMP(NX,NY), D1PTH(NPTHOUT,NPTHLEV)  !! NetCDF restart is in Double Precision
!================================================
CFILE=TRIM(CRESTSTO)
...
USE YOS_CMF_MAP,        ONLY: NPTHOUT,     NPTHLEV
USE YOS_CMF_PROG,       ONLY: D2RIVSTO,    D2FLDSTO,    B2RIVOUT_PRE,B2FLBOUT_PRE, &
                            & B1PTHFLW_PRE,B2RIVDPH_PRE,B2FLDSTO_PRE,B2GDWSTO, &
                            & D2DAMSTO,    D2LEVSTO
USE CMF_UTILS_MOD,      ONLY: INQUIRE_FID
IMPLICIT NONE
!* local variable
...
REAL(KIND=JPRD)            :: D2TMP(NSEQMAX,1)        !! use Real*8 for code simplicity
REAL(KIND=JPRD)            :: D1PTH(NPTHOUT,NPTHLEV) 
REAL(KIND=JPRM)            :: R1PTH(NPTHOUT,NPTHLEV) 
!================================================
!*** set file nam
...
RIREC=0
  D2TMP=D2RIVSTO
   CALL WRTE_BIN_MAP(D2TMP,TMPNAM,RIREC)
  D2TMP=D2FLDSTO
   CALL WRTE_BIN_MAP(D2TMP,TMPNAM,RIREC)
!!================
...
USE CMF_UTILS_MOD,      ONLY: VECD2MAPD,  VECD2MAPR
USE YOS_CMF_MAP,        ONLY: REGIONTHIS, NSEQMAX
#ifdef UseMPI_CMF
USE CMF_CTRL_MPI_MOD,   ONLY: CMF_MPI_REDUCE_R2MAP, CMF_MPI_REDUCE_D2MAP
#endif
IMPLICIT NONE
REAL(KIND=JPRD)            :: D2VAR(NSEQMAX,1)  !! use Real*8 for code simplicity
INTEGER(KIND=JPIM)         :: TNAM,IREC
!* local
REAL(KIND=JPRM)            :: R2TEMP(NX,NY)
REAL(KIND=JPRD)            :: D2TEMP(NX,NY)
...
#ifdef UseCDF_CMF
USE NETCDF
USE YOS_CMF_INPUT,      ONLY: BMIS
USE YOS_CMF_TIME,       ONLY: KMINNEXT, KMINSTART, ISYYYY,ISMM,ISDD, ISHOUR, ISMIN
USE YOS_CMF_TIME,       ONLY: JYYYYMMDD,JHOUR
USE YOS_CMF_MAP,        ONLY: B1LON,    B1LAT,     REGIONTHIS, NSEQMAX
USE CMF_UTILS_MOD,      ONLY: NCERROR,  VECD2MAPD
#ifdef UseMPI_CMF
...
REAL(KIND=JPRD)            :: D2VEC(NSEQMAX,1), D2TEMP(NX,NY), D1PTH(NPTHOUT,NPTHLEV)
!================================================
!*** 1. set file name & tim
...
IF( REGIONTHIS==1 )THEN   !! write restart only on master node
  ...
  CALL NCERROR( NF90_PUT_ATT(NCID, VARID, '_FillValue',REAL(BMIS,KIND=JPRD)),'in here?' )
  ...
  CALL NCERROR( NF90_PUT_ATT(NCID, VARID, 'units',"m3") )
  CALL NCERROR( NF90_PUT_ATT(NCID, VARID, '_FillValue',REAL(BMIS,KIND=JPRD)) )
  ...
  CALL NCERROR( NF90_PUT_VAR(NCID,VARID,B1LON))
  
  CALL NCERROR ( NF90_INQ_VARID(NCID,'lat',VARID),'getting id' )
  CALL NCERROR( NF90_PUT_VAR(NCID,VARID,B1LAT))
...
DO JF=1,9
  ...
  SELECT CASE(JF)
    CASE (1)
      CVAR='rivsto'
      CALL VECD2MAPD(D2RIVSTO,D2TEMP)
      IOUT=1
    CASE (2)
      CVAR='fldsto'
      CALL VECD2MAPD(D2FLDSTO,D2TEMP)
      IOUT=1
    CASE (3)
      CVAR='rivout_pre'
      IF( .not. LSTOONLY ) THEN
        D2VEC(:,:)=B2RIVOUT_PRE(:,:)
        CALL VECD2MAPD(D2VEC,D2TEMP)
        IOUT=1
      ENDIF
    CASE (4)
      CVAR='fldout_pre'
   ...
   CASE (5)
    ...
    CASE (7)
      CVAR='gdwsto'
      IF( LGDWDLY ) THEN
        CALL VECD2MAPD(B2GDWSTO,D2TEMP)
        IOUT=1
      ENDIF
    CASE (8)  !!! LDAMOUT
      CVAR='damsto'
      IF( LDAMOUT ) THEN
        CALL VECD2MAPD(D2DAMSTO,D2TEMP)  !! D2DAMSTO only allocated for LDAMOUT
        IOUT=1
      ENDIF
    CASE (9)  !!! LLEVEE
      CVAR='levsto'
      IF( LLEVEE ) THEN
        CALL VECD2MAPD(D2LEVSTO,D2TEMP)  !! D2DAMSTO only allocated for LDAMOUT
        IOUT=1
      ENDIF
  END SELECT
...
IF ( LPTHOUT ) THEN
  IF ( .not. LSTOONLY )THEN
    D1PTH(:,:)=B1PTHFLW_PRE(:,:)  !! convert Float32 to Float64 (for Single Precison Use)
#ifdef UseMPI_CMF
    CALL CMF_MPI_REDUCE_D1PTH(D1PTH)
#endif
    IF( REGIONTHIS==1 )THEN
      CALL NCERROR( NF90_INQ_VARID(NCID,'pthflw_pre',VARID))
      CALL NCERROR( NF90_PUT_VAR(NCID,VARID,D1PTH(:,:),(/1,1,1/),(/NPTHOUT,NPTHLEV,1/)) )
    ENDIF
  ENDIF
ENDIF
