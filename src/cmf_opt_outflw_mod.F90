...
USE YOS_CMF_PROG,    ONLY: B2RIVOUT_PRE, B2FLDOUT_PRE, B1PTHFLW_PRE ,B2RIVDPH_PRE         !! output
...
!=== Floodplain Flow ===
  ...
  IF( BARE_F>1.E-5 )THEN 
    B2FLDOUT_PRE(ISEQ,1) = BARE_F * ( PMANFLD**(-1.) * BFLW_F**(2./3.) * abs(BSLOPE_F)**(0.5) )
    IF( BSLOPE_F<0._JPRB ) B2FLDOUT_PRE(ISEQ,1)=-B2FLDOUT_PRE(ISEQ,1)
  ELSE
    B2FLDOUT_PRE(ISEQ,1) = 0._JPRB
  ENDIF
END DO
!$OMP END PARALLEL DO
