...
SUBROUTINE PRINT_NAMELISTS(NAME)
  USE CMF_CTRL_BOUNDARY_MOD, ONLY : NBOUND
  USE CMF_CTRL_DAMOUT_MOD, ONLY : NDAMOUT
  USE CMF_CTRL_FORCING_MOD, ONLY : NFORCE
  USE CMF_CTRL_LEVEE_MOD, ONLY : NLEVEE
  USE CMF_CTRL_MAPS_MOD, ONLY : NMAP
  ! this module : NRUNVER
  ! this module : NDIMTIME
  ! this module : NPARAM
  USE CMF_CTRL_OUTPUT_MOD, ONLY : NOUTPUT
  USE CMF_CTRL_RESTART_MOD, ONLY : NRESTART
  USE CMF_CTRL_TIME_MOD, ONLY : NSIMTIME
  USE CMF_CTRL_TRACER_MOD, ONLY : NAMTRACE

  IMPLICIT NONE
  CHARACTER(LEN=*), INTENT(IN), OPTIONAL :: NAME
  LOGICAL :: PALL = .TRUE.

  IF (PRESENT(NAME)) PALL = .FALSE.
  IF (PALL .OR. NAME == "NBOUND") WRITE(LOGNAM, NML=NBOUND)
  IF (PALL .OR. NAME == "NDAMOUT") WRITE(LOGNAM, NML=NDAMOUT)
  IF (PALL .OR. NAME == "NFORCE") WRITE(LOGNAM, NML=NFORCE)
  IF (PALL .OR. NAME == "NLEVEE") WRITE(LOGNAM, NML=NLEVEE)
  IF (PALL .OR. NAME == "NMAP") WRITE(LOGNAM, NML=NMAP)
  IF (PALL .OR. NAME == "NRUNVER") WRITE(LOGNAM, NML=NRUNVER)
  IF (PALL .OR. NAME == "NDIMTIME") WRITE(LOGNAM, NML=NDIMTIME)
  IF (PALL .OR. NAME == "NPARAM") WRITE(LOGNAM, NML=NPARAM)
  IF (PALL .OR. NAME == "NOUTPUT") WRITE(LOGNAM, NML=NOUTPUT)
  IF (PALL .OR. NAME == "NRESTART") WRITE(LOGNAM, NML=NRESTART)
  IF (PALL .OR. NAME == "NSIMTIME") WRITE(LOGNAM, NML=NSIMTIME)
  IF (PALL .OR. NAME == "NAMTRACE") WRITE(LOGNAM, NML=NAMTRACE)
