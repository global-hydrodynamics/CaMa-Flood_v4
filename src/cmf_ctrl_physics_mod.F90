...
USE YOS_CMF_INPUT,         ONLY: LKINE,  LSLPMIX, LFLBOUT,   LPTHOUT,   LDAMOUT, LLEVEE, LOUTINS
USE YOS_CMF_PROG,          ONLY: B2FLBOUT, B2FLBOUT_PRE
!
...
!=== 1. Calculate river discharge 
  ...
  IF( .not. LFLBOUT )THEN
    B2FLBOUT(:,:)=0._JPRB    !! OPTION: no high-water channel flow
...
DO ISEQ=1, NSEQRIV
  DDPH=MAX(B2RIVDPH(ISEQ,1),0.01_JPRB )
  DDST=B2NXTDST(ISEQ,1)
...
USE YOS_CMF_DIAG,            ONLY: DGLBSTOPRE2,DGLBSTONEW2,DGLBRIVSTO,DGLBFLDSTO,DGLBFLBARE
...
WRITE(LOGNAM,'(I4.4,4(A1,I2.2),I6,a6,3F12.3,G12.3,2x,2F12.3,a6, 2F12.3,G12.3,3F12.3)') &
  ...
  DGLBSTOPRE2*DORD,DGLBSTONEW2*DORD,DERROR2*DORD,    DGLBRIVSTO*DORD,DGLBFLDSTO*DORD, DGLBFLBARE*DORD
...
USE YOS_CMF_PROG,            ONLY: B2RIVOUT,     B2FLBOUT,     D2FLDSTO
USE YOS_CMF_PROG,            ONLY: B2RIVOUT_PRE, B2FLBOUT_PRE, B2FLDSTO_PRE, B2RIVDPH_PRE
USE YOS_CMF_DIAG,            ONLY: B2RIVDPH
IMPLICIT NONE
INTEGER(KIND=JPIM),SAVE         :: ISEQ
! ================================================
...
DO ISEQ=1, NSEQALL ! for river mouth
  B2RIVOUT_PRE(ISEQ,1)=B2RIVOUT(ISEQ,1)                              !! save outflow (t)
  B2RIVDPH_PRE(ISEQ,1)=B2RIVDPH(ISEQ,1)                              !! save depth   (t)
  B2FLBOUT_PRE(ISEQ,1)=B2FLBOUT(ISEQ,1)                              !! save outflow (t)
  B2FLDSTO_PRE(ISEQ,1)=D2FLDSTO(ISEQ,1)
END DO
!$OMP END PARALLEL DO
