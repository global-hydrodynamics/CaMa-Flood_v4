...
USE CMF_UTILS_MOD,           ONLY: INQUIRE_FID, NCERROR, MAPR2VECB, MAPI2VECI
USE NETCDF
IMPLICIT NONE
...
CALL NCERROR ( NF90_GET_VAR(NCID,VARID,R2TMP,(/1,1,1/),(/NX,NY,INPN/)),'reading data' ) 
DO INPI=1, INPN
  CALL MAPR2VECB(R2TMP(:,:,INPI:INPI),INPA(:,INPI:INPI))
END DO
DEALLOCATE( R2TMP )
...
USE CMF_UTILS_MOD,           ONLY: INQUIRE_FID, CONV_END,  CONV_ENDI, MAPR2VECB, MAPI2VECI
USE YOS_CMF_MAP,             ONLY: NSEQMAX
IMPLICIT NONE
INTEGER(KIND=JPIM)              :: INPI
INTEGER(KIND=JPIM),ALLOCATABLE  :: I2TMP(:,:)
...
DO INPI=1, INPN
  ...
  READ(TMPNAM,REC=2*INPN+INPI) R2TMP
   CALL MAPR2VECB( R2TMP,INPA(:,INPI:INPI))
END DO
...
USE CMF_UTILS_MOD,           ONLY: VECB2MAPB
#ifdef UseMPI_CMF
USE CMF_CTRL_MPI_MOD,        ONLY: CMF_MPI_REDUCE_B2MAP
#endif
USE YOS_CMF_DIAG,            ONLY: B2FLDFRC
USE YOS_CMF_INPUT,           ONLY: NX,NY
IMPLICIT NONE 
! Declaration of arguments 
REAL(KIND=JPRB)                  :: B2MAPTMP(NX,NY)
REAL(KIND=JPRB), INTENT(OUT)     :: PBUFF(:,:,:)
!============================
CALL VECB2MAPB(B2FLDFRC,B2MAPTMP)             !! MPI node data is gathered by VECB2MAPR
#ifdef UseMPI_CMF
  CALL CMF_MPI_REDUCE_B2MAP(B2MAPTMP)
#endif

CALL INTERPI(B2MAPTMP,PBUFF(:,:,1))        !!  Inverse interpolation (CaMa grid -> input runoff grid)
...
USE YOS_CMF_PROG,            ONLY: B2RUNOFF,B2ROFSUB,B2WEVAP
IMPLICIT NONE 
! Declaration of arguments 
...
IF (LINTERP) THEN ! mass conservation using "input matrix table (inpmat)"
  CALL ROFF_INTERP(PBUFF(:,:,1),B2RUNOFF)
  IF (LROSPLIT) THEN
    CALL ROFF_INTERP(PBUFF(:,:,2),B2ROFSUB)
  ELSE
    B2ROFSUB(:,:) = 0._JPRB
  ENDIF
ELSE !  nearest point
  CALL CONV_RESOL(PBUFF(:,:,1),B2RUNOFF)
  IF (LROSPLIT) THEN
    CALL CONV_RESOL(PBUFF(:,:,2),B2ROFSUB)
  ELSE
    B2ROFSUB(:,:) = 0._JPRB
  ENDIF
ENDIF 
...
IF (LWEVAP) THEN
  IF ( SIZE(PBUFF,3) == 3 ) THEN
    CALL ROFF_INTERP(PBUFF(:,:,3),B2WEVAP)
  ELSE
...
USE CMF_UTILS_MOD,           ONLY: MAPB2VECB
IMPLICIT NONE
...
CALL MAPB2VECB(PBUFFIN,B2TEMP)
!$OMP PARALLEL DO
DO ISEQ=1, NSEQALL
  IF( B2TEMP(ISEQ,1).NE.RMIS )THEN
    PBUFFOUT(ISEQ,1) = B2TEMP(ISEQ,1) * B2GRAREA(ISEQ,1) / DROFUNIT
    PBUFFOUT(ISEQ,1) = MAX(PBUFFOUT(ISEQ,1), 0._JPRB)
  ELSE
    PBUFFOUT(ISEQ,1)=0._JPRB
  ENDIF
