...
USE YOS_CMF_INPUT,           ONLY: NX, NY, INPN, LMAPEND
USE CMF_UTILS_MOD,           ONLY: INQUIRE_FID
...
!OPEN(TMPNAM,FILE=CINPMAT,FORM='UNFORMATTED',ACCESS='DIRECT',RECL=4*NX*NY*INPN)  !! for super-high-resolution simulation: NX*NY*NINP < integer limit)
...
DO INPI=1, INPN
  READ(TMPNAM,REC=       INPI) INPX(:,:,INPI:INPI)
  READ(TMPNAM,REC=  INPN+INPI) INPY(:,:,INPI:INPI)
  READ(TMPNAM,REC=2*INPN+INPI) R2TMP(:,:,INPI:INPI)
END DO
