...
USE YOS_CMF_PROG,       ONLY: B2RIVOUT,  B2FLDOUT,  D2RIVSTO,  D2FLDSTO,  B2RUNOFF
...
DO ISEQ=1, NSEQALL
  ...
  DGLBRIVOUT = DGLBRIVOUT + B2RIVOUT(ISEQ,1)*DT + B2FLDOUT(ISEQ,1)*DT + B2PTHOUT(ISEQ,1)*DT
  ...
  D2FLDSTO(ISEQ,1) = D2FLDSTO(ISEQ,1) + B2FLDINF(ISEQ,1)*DT - B2FLDOUT(ISEQ,1)*DT &
  ...
  B2OUTFLW(ISEQ,1)=B2RIVOUT(ISEQ,1)+B2FLDOUT(ISEQ,1)+B2PTHOUT(ISEQ,1)
